...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& inputcloud) {
    int cloudFlag = 0;
    // cout << "inputcloud" << inputcloud << endl;

    // if(!inputcloud){
    //     cout << "fuck" << endl;
    //     std_msgs::String msg;
    //     msg.data = "-1";
    //     clusterPub.publish(msg);
    //     return;
    // }
    ...
    for (const auto& cluster_index : cluster_indices) {
        ...
        for (const auto& point_index : cluster_index.indices) {
            // cout << cluster_index.indices << endl;
            pcl::PointXYZI point;
            ...
            totalcloud_clustered.push_back(point);
    ...
    if (cloudFlag == 0){
        ...
        empty_cloud_msg.header.frame_id = "velodyne123123213";
        clusterPub.publish(empty_cloud_msg);
    } 
