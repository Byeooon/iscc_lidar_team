...
# 카메라의 좌표계에서 X축이 왼쪽, Y축이 위, Z축이 앞을 가리킨다고 가정

# Yaw (Z축) 회전: 왼쪽으로 90도 틀어짐
# Pitch (Y축) 회전: 아래쪽으로 약간 기울어짐 (-10도 예시)
# Roll (X축) 회전: 좌우 기울어짐 없음
...
class create_matrix:
    ...
    def transformMTX_lidar2cam(self, params_lidar, params_cam):
        ...
        R_T = np.matmul(self.rotationMtx(0., np.deg2rad(90.), 0.), R_T ) # 왼쪽 방향으로 카메라 틈
        R_T = np.matmul(self.rotationMtx(0.,0., np.deg2rad(0.)), R_T ) # 아래 방향으로 카메라 틈
        # R_T = np.matmul(self.rotationMtx(np.deg2rad(0.), np.deg2rad(0.), 0.), self.translationMtx(x_rel, y_rel, z_rel))
...
class LIDAR2CAMTransform:
    def __init__(self,params_cam,params_lidar):
        ...
        self.image_pub = rospy.Publisher("/sensor_fusion_result_left",Image,queue_size= 1)
        self.clustering_point_pub_left = rospy.Publisher("/final_point_clustering_left",clustering_points,queue_size= 1)
        self.yolo_sub = rospy.Subscriber("/traffic_cone_yolo_left_result",Image,self.callback_image)
        self.cluster_sub = rospy.Subscriber("/cluster",PointCloud2,self.callback_lidar)
        self.bbox_sub = rospy.Subscriber("/traffic_cone_bbox_class_left",YoloTrafficCone,self.callback_bbox)
    ...
    def callback_image(self,msg):        
        ...
        if 'point_x' not in globals():
            print("left, global variable 'center : point_x' is not defined")
            return
    ...
    def callback_bbox(self,msg):
        ...
        print("msg ", msg)
        if 'point_x' not in globals():
            print("left, global variable 'center_bbox : point_x' is not defined")
            return
        ...
        if msg.boxes == []:
            ...
            if (len(cluster_msg.x) == 0):
                ...
                cluster_msg.label = []
        # print("?!>!!?!!?!?!!?", cluster_msg)                
        self.clustering_point_pub_left.publish(cluster_msg)
    ...
    def draw_pts_img(self, img, xi, yi):
        ...
        # frame = cv2.circle(frame, (320, 240), 5, (255,0,0),-1)
        return frame

if __name__ == '__main__':
    rospy.init_node("webcam_pub" , anonymous = True)
    params_cam = {"WIDTH": 640, "HEIGHT": 480, "FOV": 78, "X": 0.03, "Y": 0.016, "Z": -0.008} # erp42 , left > 0
