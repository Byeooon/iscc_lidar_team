...
class YoloV7:
    def __init__(self):
        ...
        self.weights_path = '/home/foscar/sensor_fusion_2024/src/src/camera/weight/rubbercone20.pt'
...
class Yolov7Publisher:
    def __init__(self):
        self.class_labels = ["yellow_cone", "blue_cone"]
        self.img_size = (640, 480)

        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        

        self.visualization_left_publisher = rospy.Publisher(
            "/traffic_cone_yolo_left_result", Image, queue_size= 1
        )
        ...
        self.subscriber_left = rospy.Subscriber(
            "/usb_cam_left/image_raw", Image, self.process_img_msg_left
        )

        self.boundingbox_publisher = rospy.Publisher("/traffic_cone_bbox_class_left", YoloTrafficCone, queue_size=1)
    ...
    def publish_boundingbox(self, bboxes, classes):
        ...
        # print("yolo left bbox ", msg)
        self.boundingbox_publisher.publish(msg)


    def process_img_msg_left(self, img_msg):
        ...
        for bbox, class_id in zip(detections[:, :4], detections[:, 5]):
            ...
            # print(f"  Class: {class_name}, Bounding Box: ({x1}, {y1}) to ({x2}, {y2})")
        ...
        self.visualization_left_publisher.publish(vis_msg)
